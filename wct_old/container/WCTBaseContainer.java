...
public abstract class WCTBaseContainer extends Container {
	...
	protected void bindPlayerInventory(final InventoryPlayer inventoryPlayer, final int offsetX, final int offsetY) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 9; j++) {
				if (locked.contains(j + i * 9 + 9)) {
					addSlotToContainer(new SlotDisabled(new InvWrapper(inventoryPlayer), j + i * 9 + 9, 8 + j * 18 + offsetX, offsetY + i * 18));
				}
				else {
					addSlotToContainer(new SlotPlayerInv(new InvWrapper(inventoryPlayer), j + i * 9 + 9, 8 + j * 18 + offsetX, offsetY + i * 18));
				}
			}
		}
		...
		for (int i = 0; i < 9; i++) {
			if (locked.contains(i)) {
				addSlotToContainer(new SlotDisabled(new InvWrapper(inventoryPlayer), i, 8 + i * 18 + offsetX, 58 + offsetY));
			}
			else {
				addSlotToContainer(new SlotPlayerHotBar(new InvWrapper(inventoryPlayer), i, 8 + i * 18 + offsetX, 58 + offsetY));
			}
		}
	}
	...
	public void swapSlotContents(final int slotA, final int slotB) {
		...
		// something to do?
		if (isA == ItemStack.EMPTY && isB == ItemStack.EMPTY) {
			return;
		}
