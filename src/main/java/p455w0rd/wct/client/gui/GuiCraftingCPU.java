...
public class GuiCraftingCPU extends WCTBaseGui implements ISortSource {
	...
	public void drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
		...
		fontRendererObj.drawString(title, TITLE_LEFT_OFFSET, TITLE_TOP_OFFSET, TEXT_COLOR);
		...
		for (int z = viewStart; z < Math.min(viewEnd, visual.size()); z++) {
			...
			if (refStack != null) {
				...
				if (stored != null && stored.getStackSize() > 0) {
					...
					fontRendererObj.drawString(str, (int) ((x * (1 + SECTION_LENGTH) + ITEMSTACK_LEFT_OFFSET + SECTION_LENGTH - 19 - (w * 0.5)) * 2), (y * offY + ITEMSTACK_TOP_OFFSET + 6 - negY + downY) * 2, TEXT_COLOR);
